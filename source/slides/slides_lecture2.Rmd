...
```{r priorpredviz, cache =F, eval = TRUE, echo = FALSE, fig.asp = .7, dpi = 150, out.width=500, warning = F}
prior.pred.means <- colMeans(t(prior.predictions))
prior.pred.sd <- apply(t(prior.predictions), 2, sd)
...
- Each should ideally be at least 400, 100 at a minimum.
...
- To assess how well a model reflects key patterns in the data, one typically compares it to one or more models with alternative specifications.
...
.footnote[Table from Haaf & Rouder (2019) *Psychonomic Bulletin and Review*.]
...
- Formally (assuming past and future data are independent): $p(y_{pred} | y) = \int_\theta p(y_{pred} | \theta) p(\theta | y) d \theta$
--
...
```{r postpredviz, cache =F, eval = TRUE, echo = FALSE, fig.asp = .5, dpi = 150, out.width=750, warning = F}
prior.pred.means <- colMeans(t(prior.predictions))
prior.pred.sd <- apply(t(prior.predictions), 2, sd)
...
pred <- data.frame(
  Summary = rep(c("Prior_mean", "Prior_sd", "Posterior_mean", "Posterior_sd"), each = 8000)
  , Prediction = c(prior.pred.means, prior.pred.sd, post.pred.means, post.pred.sd))

ggplot(pred, aes(x = Prediction, fill = Summary)) +
  geom_histogram (binwidth = 0.03, alpha = 0.5, position = "identity") +
  facet_wrap(~ Summary, ncol = 2, nrow = 2) +
  xlim(c(-.05, .75)) + 
  theme_light(base_size = 16)
```
